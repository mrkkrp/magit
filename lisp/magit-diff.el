...
(defun magit-revision-hideshow-header-line ()
  (setq header-line-format
        (and (not (pos-visible-in-window-p 1))
             ;; TODO avoid flickering by making initial space "fringe-width"
             (propertize (concat " Commit " (magit-rev-parse (car magit-refresh-args)))
                         'face 'magit-header-line))))
  ...
  (let (children)
    ...
    (magit-bind-match-strings (rev refs) nil
      ...
      (magit-insert-section (headers)
        (magit-insert-heading
          (propertize (concat "Commit " rev "\n") ; TODO where's my underline?
                      'face '(magit-header-line header-line)))
        ;; TODO why does this give a *worse* result?
